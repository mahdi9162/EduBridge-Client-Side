...
const Btn = ({ children, className = '', ...rest }) => {
  return (
    <>
      <button
        className={`relative inline-flex items-center justify-center overflow-hidden tracking-tighter text-white bg-primary rounded-md cursor-pointer group ${className}`}
        {...rest}
      >
        ...
        <span className="relative text-sm md:text-base font-semibold">{children}</span>
      </button>
    </>
  );
};
