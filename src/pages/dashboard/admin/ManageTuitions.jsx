        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mt-10 max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-5">
            {pendingTuitions.length === 0 ? (
              <div className="lg:col-span-2 bg-base-100 rounded-3xl border border-base-200 shadow-[0_18px_45px_rgba(15,26,51,0.08)] px-5 py-7 md:px-8 md:py-8 text-center">
                <div className="flex flex-col items-center gap-2">
                  <span className="text-3xl md:text-4xl animate-bounce">🎉</span>
                  <h3 className="text-base md:text-xl font-semibold text-base-content">No pending tuitions right now</h3>
                </div>

                <p className="mt-2 text-xs md:text-sm text-neutral max-w-xs md:max-w-md mx-auto">
                  You’re all caught up. New tuition requests will appear here for review.
                </p>

                <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-base-200 bg-base-200/40 px-3 py-1.5 text-[11px] md:text-xs text-neutral">
                  <span className="animate-pulse">🕒</span>
                  Waiting for new submissions
                </div>

                <div className="mt-4 text-[11px] md:text-xs text-neutral">
                  Tip: Check again in a bit <span className="animate-pulse">✨</span>
                </div>
              </div>
            ) : (
              pendingTuitions.map((tuition, i) => (
                <div key={i} className="bg-base-100 rounded-3xl border border-base-200 shadow-[0_18px_45px_rgba(15,26,51,0.08)] p-5 md:p-7">
                  {/* Card Header */}
                  <div className="flex items-start justify-between gap-4">
                    <h3 className="text-lg md:text-xl font-semibold text-base-content line-clamp-1">{tuition.title}</h3>
                    <span className="badge badge-soft badge-warning text-xs px-3 py-1">{tuition.postStatus}</span>
                  </div>

                  {/* Tags */}
                  <div className="mt-3 flex flex-wrap gap-2">
                    <span className="px-3 py-1 rounded-full bg-accent/70 text-xs md:text-sm text-base-content">{tuition.classLevel}</span>
                    <span className="px-3 py-1 rounded-full bg-accent/70 text-xs md:text-sm text-base-content">{tuition.subject}</span>
                    <span className="px-3 py-1 rounded-full bg-accent/70 text-xs md:text-sm text-base-content">{tuition.location}</span>
                  </div>

                  <div className="mt-5 border-t border-base-200" />

                  {/* Meta */}
                  <div className="mt-5 flex flex-col md:flex-row md:items-center md:justify-between gap-2 text-sm text-neutral">
                    <p>
                  ...
                  <p className="mt-3 text-[11px] text-neutral text-center md:text-left">
